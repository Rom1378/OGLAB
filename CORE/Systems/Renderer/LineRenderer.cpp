...
namespace LineRenderer {
	...
	std::vector<line_vertex> DrawData = {};
	...
	void draw(glm::vec3 const& start, glm::vec3 const& end, glm::vec4 const& color) {
		...
		DrawData.push_back(startv);
		DrawData.push_back(endv);
	...
	void LineRenderer::render() {
		...
		if (count > allocatedLines) {
			...
			glBufferData(GL_ARRAY_BUFFER, allocatedLines * sizeof(line_vertex)*2, &DrawData[0], GL_DYNAMIC_DRAW);
		}
		else if (count > 0) {
			glBindBuffer(GL_ARRAY_BUFFER, VBO);
			glBufferSubData(GL_ARRAY_BUFFER, 0, count * sizeof(line_vertex)*2, &DrawData[0]);
		}
		...
		lineShader->setMat4("view", glm::value_ptr(Renderer::getView()));
		lineShader->setMat4("projection", glm::value_ptr(Renderer::getProjection()));
